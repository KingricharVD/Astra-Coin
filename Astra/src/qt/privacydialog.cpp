                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zAstra ought to be enough for anybody." - Bill Gates, 2017
    ui->zAstrapayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAstraSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzAstra->setEnabled(false);
        ui->pushButtonMintzAstra->setToolTip(tr("zAstra is currently disabled due to maintenance."));

        ui->pushButtonSpendzAstra->setEnabled(false);
        ui->pushButtonSpendzAstra->setToolTip(tr("zAstra is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAstrapayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzAstra_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAstra is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAstra...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAstra in ") + 
...
void PrivacyDialog::on_pushButtonSpendzAstra_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAstra is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zAstra
        sendzAstra();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAstra
    sendzAstra();
}

void PrivacyDialog::on_pushButtonZAstraControl_clicked()
{
    ZAstraControlDialog* zAstraControl = new ZAstraControlDialog(this);
    zAstraControl->setModel(walletModel);
    zAstraControl->exec();
}

void PrivacyDialog::setZAstraControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAstraSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzAstra()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Astra Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAstrapayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAstra is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " Astra </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAstra</b>";
    ...
    // use mints from zAstra selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAstraControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAstraControlDialog::GetSelectedMints();
    }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zAstra transaction
    ...
    // Clear zAstra selector in case it was used
    ZAstraControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAstra Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Astra, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAstra Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAstra </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAstra "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAstra "));
    ui->labelzAstraAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzAstraSyncStatus->setVisible(fShow);
}
