...
#include "ui_zAstracontroldialog.h"
...
std::list<std::string> ZAstraControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAstraControlDialog::listMints;

ZAstraControlDialog::ZAstraControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAstraControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAstraControlDialog::~ZAstraControlDialog()
{
    delete ui;
}

void ZAstraControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAstraControlDialog::updateList()
{
...
void ZAstraControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAstraControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAstra_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAstraControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAstraControlDialog::GetSelectedMints()
{
...
void ZAstraControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
