...
Value GetNetworkHashPS(int lookup, int height)
{
    ...
    // In case there's a situation where minTime == maxTime, we don't want a Astrade by zero exception.
    if (minTime == maxTime)
        return 0;
...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or Astra.conf setting gen)\n"
