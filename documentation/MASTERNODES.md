...
__Masternodes__ are computers that run a Astra wallet and perform utility functions for network such as storing metadata and processing special transactions.

__Masternode collateral__ amount that is needed to run a masternode, each masternode wallet needs to lock a certain amount of Astra in the wallet in order to register as a masternode in the network. 
...
## Supporting the Astra Network

What is a masternode, really?   It's just a piece of software.  It's an app that's already built in to the Astra desktop wallet.  You launch the app, "fund" it with some of your Astra coins, and it will go to work once it's connected with enough other nodes.  Those other nodes are owned by other Astra masternode holders around the world, who have gone through the same process. 

What does a masternode actually do?  Astra's masternodes remain in constant contact with other nodes in the network.   Through this communication they verify and process transactions and keep everything secure, while continuously maintaining an up-to-date copy of the ledger.  Approximately every minute, one of the masternodes wins the right to add the latest block to the Astra blockchain.  That winner collects all the transactions, writes them into the block, and gets paid in Astra.  Other masternodes verify everything, and if all is good, the blockchain grows.  

Masternode holders aren't getting Astra for free.  They are paid for the work they're doing, which requires the following:
...
* *Cloud Install* - You don't need a computer for this.  You can host a masternode in the cloud with our system.  There's a small monthly fee, which is deducted from your account, in Astra, automatically each month.
...
* *Software Updates* - Astra makes this easy with automatic updates
* Staying current on Astra news.  Participating in our ecosystem.  Voting in our governance.

What happens if the masternode goes down?  Perhaps power is lost or a computer needs to be repaired.  For whatever reason, it will disconnect from the network, and during this time no rewards can be earned.  Once re-connected, it will download the latest version of the Astra ledger and will start earning coins again.  
